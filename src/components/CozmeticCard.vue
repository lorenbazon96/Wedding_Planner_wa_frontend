<template>
  <div class="cozmetic-card mb-3 rounded-4 overflow-hidden">
    <div class="row g-0">
      <div class="col-12 col-md-4">
        <img
          :src="store.gallery && store.gallery.length ? store.gallery[0] : 'https://via.placeholder.com/420x200'"
          class="w-100 h-100 store-img"
          alt="cozmetic salon"
        />
      </div>
      <div class="col-12 col-md-8">
        <div class="p-3 h-100 d-flex flex-column justify-content-between">
          <div>
            <h5 class="store-title">{{ store.name }}</h5>
            <div class="mb-1">
              <span class="text-warning">★ ★ ★ ★ ★</span>
              <small class="text-muted ms-2">{{ store.rating }}</small>
            </div>
            <p class="mb-1">📍 {{ store.address }}</p>
            <p class="mb-1">📞 {{ store.phone }}</p>
            <p class="mb-2">📧 {{ store.email }}</p>
          </div>
          <div>
            <button
              class="btn btn-dark rounded-pill px-4 py-1"
              @click="$emit('read-more', store)"
            >
              Read more...
            </button>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: "CozmeticCard",
  props: {
    store: { type: Object, required: true },
  },
  emits: ["read-more"],
};
</script>

<style scoped>
.cozmetic-card {
  background: #fff;
  box-shadow: 0 6px 14px rgba(0, 0, 0, 0.08);
}
.store-img {
  object-fit: cover;
  min-height: 150px;
}
.store-title {
  font-family: Georgia, serif;
}
</style>
